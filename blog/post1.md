...
最后第三章是实现篇，实现篇会直接继承理论分析的结果，将它们转化成对应的 CUDA Kernel（文中是以伪代码的形式呈现）。作为例子，我们会呈现三种不同的设计方案，三种设计方案其实还是围绕着减少上述 RAW、WAR 这两类停顿而展开。
...
不论流水线怎么编排，数据流动的总体步骤都是一样的，不同方案的区别在于同步方案、数据读写的次序和粒度、tile sizes 的配置、各种 buffer 的数量和大小等等。为了能把这些区别讲清楚，我们先建立一个数据流模型。
...
既然我们说，流水线调度的根本目标是减少 RAW 停顿和 WAR 停顿，但是前面我们又提高了提高算术强度，这俩者到底什么关系？事实上，提高算术强度本质上依然是减少了 RAW 停顿 —— 通过减少对内存（或 L2）的访问，更好地复用片上的数据，这本质上是使得 RAW 停顿减少了。
...
* 18 个尺寸里 16 个为正、1 个持平、1 个倒退，平均 +0.7%，最大 +2.0%。收益是实的，
  但不大 —— 第三种设计已经把大部分 WAR 停顿掩盖掉了，这里拿到的是剩下的那一点。
  取每个尺寸的最优配置，18 个尺寸里有 12 个跑赢 cuBLAS；其中 12 次由 BK=128 +
  GSM=16 拿下，它仍然是单一最优的一档。
* 收益的来源可以直接从设计上读出来：第三种设计的 MMA 循环是「panel 0 一个 k tile、
  panel 1 一个 k tile」交替，所以在 k=0 处 issue 完 panel 0 的一个 k tile 就要等
  panel 1 的 accumulator，能用来掩盖 panel 1 draining 的工作只有 1 个 k tile；而
  两轮轮转的批量是 NS，panel 0 一次能连发 NS 个 k tile。换句话说，NS 在这里同时兼了
  两个职：既是 ring 的深度（对付 RAW 停顿），也是掩盖第二个 panel draining 的本钱
  （对付 WAR 停顿）。
* **BK=128 上把同配置逐格相减，能看到这个机制的边界**。小尺寸（3072³–6144³）平均
  +2.7% 到 +3.2%，比 BK=64 的 +0.6% 到 +1.0% 大得多：那里瓶颈就是 ring 深度，
  NS 从 2 变成 3 直接减少 RAW 停顿。大尺寸（17408³–20480³）反过来 —— GSM=8 平均
  -4.7%，GSM=12 平均 -2.6%，只有 GSM=16 才回到 +0.2%。大尺寸的瓶颈已经不是流水线
  深度而是 L2 复用，而两轮轮转改变了 B panel 的访问次序，浅 swizzle 下这个次序更
  伤 L2，要靠更深的 swizzle 补回来。
* 这一点值得强调：**同一个改动在不同尺寸上作用于不同的项**。它加深 ring（改善 f），
  同时扰动访存次序（也作用于 f，但方向相反）。哪一项占上风取决于当前的瓶颈是哪个，
  所以「两轮轮转更好」这句话离开尺寸和 GSM 就没有意义。
* 完整数字（含同一次运行的第三种设计对照表，以及同配置逐格相减的表）见
  [blog/perf-data.md](./perf-data.md)。
