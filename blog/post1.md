...
可以看到 BK 的选择存在一个交叉点：在 4096 到 6144 这个区间，BK=64 反而更快 2 到 4 个百分点，因为 BK 加倍以后 TMA buffer 从 6 个掉到 3 个，run-ahead 的深度不够，掩盖不住内存延迟；而到了 16384 以上，BK=128 开始明显占优，在 20480 上快出 **9.2%**。可以注意到真正在变化的是 BK=64 那一列 —— 它从 6144 上的 1394 一路掉到 20480 上的 1192，而 BK=128 那一列基本稳定在 1300 上下。也就是说 BK=128 的优势并不是它自己变快了，而是它在大尺寸上不像 BK=64 那样掉下去。
...
和 BN256 是同样的形状：中段 BK=64 更快，到了 16384 以上 BK=128 反超，在 20480 上快出 **5.4%**。交叉点比 BN256 稍微晚一点，幅度也小一些。原因同样能对上前面的分析：BK 翻倍以后 k 迭代次数减半，每个 output tile 的 MMA 段落变长，而 BN512 只有一个 accumulator，draining 必须在下一个 output tile 开始前完成，MMA 段落越长，这个时间窗口就越宽裕。BK=64 那一列在大尺寸上从 1410 掉到 1305，正是这个窗口不够用时卡顿的直接体现。
