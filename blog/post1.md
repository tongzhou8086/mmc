...
前三种设计里，BN 一旦选定，整个 grid 上的每一个 output tile 都用同一个 BN。这一节我们把这个前提去掉。

### 先算一笔账：wave quantization

本文所有的 kernel 都是 persistent 的：grid 固定为 `sm_count - sm_count % 2`，在 B200 上就是 148 个 CTA，也就是 **74 个 cluster** 常驻，每个 cluster 负责一块 `256 x BN` 的 output tile。于是总的 tile 数是

```
T = ceil(M / 256) * ceil(N / BN)
```

而运行时间正比于 `ceil(T / 74)` 个 wave。问题就出在这个 ceil 上：最后一个 wave 往往是不满的，那些没分到 tile 的 cluster 在这段时间里完全空转。我们把 `T / (74 * ceil(T / 74))` 称作 wave 效率，它衡量的就是这部分浪费。

对同一个尺寸，BN 不同，tile 数不同，wave 效率也就完全不同：
...
BN=512 的 tile 数只有 BN=256 的一半，粒度粗一倍，所以它更容易在最后一个 wave 上吃亏 —— 7168 就是最极端的一个，整整浪费掉 11.7%。这也解释了前面第二、三种设计在 7168 上的那个凹坑：它不是 epilogue 的问题，也不是 L2 的问题，纯粹是 tile 数除不尽 74。

### 关键一点：切 N 不需要写回中间结果

要填满最后一个 wave，就得把它拆得更细。拆的方向有两个，而它们的代价天差地别：

* **切 K**（split-K / stream-K）：同一个 output tile 的 k 循环被拆给多个 cluster，每个 cluster 只算出一部分 partial accumulation。这些 fp32 的中间结果必须写回显存，再由某一个 cluster 读回来求和 —— 一块 `256 x 512` 的 fp32 accumulator 就是 512KB，而且 TMEM 是 cluster 私有的，没有片上通路，这一趟只能走 TMEM → 寄存器 → SMEM → 显存。
* **切 N**：直接把尾部那几列用更窄的 BN 再算一遍。每一块都是**完整、独立的 output tile** —— 没有 partial accumulation，没有中间结果，没有 workspace，也不需要任何跨 cluster 的规约。

所以我们选择切 N。具体做法简单到有点无趣：主体部分用 BN=512 的 splitacc kernel 算 `C[:, 0:N_A]`，剩下的列换成 BN=256 或 BN=128 的 kernel 再发一次 —— 就是两次普通的 kernel launch，两个 kernel 都是现成的。B 和 C 的列切片只要在 TMA descriptor 里保留原本的 row pitch 就行，不需要任何拷贝。

切分点由前面那个模型直接算出来。18 个尺寸里有 9 个存在有收益的切分，其余 9 个的尾波超过半个 wave —— 拆细以后仍然要跑两趟，白搭 —— 那就老老实实跑均匀的 BN=512。

### 性能数字

同样扫了 GSM 8/12/16 与两种 BK，共 6 种选配。

![BN=512 splitacc + 自适应尾波性能对比](https://raw.githubusercontent.com/tongzhou8086/mmc/main/blog/figures/perf-bn512-hybrid.png)

这一节的对照组（同样配置、但不切尾波）与本表来自同一次运行、同一进程内交错打乱测量，所以可以逐格相减，完整数字都在 [blog/perf-data.md](./perf-data.md) 里。逐格对比下来：

| 尺寸 | 尾波 | 均匀 BN=512 | 切尾波 | 实测 | 模型预测 |
|---:|---:|---:|---:|---:|---:|
| **7168** | BN=256 | 1335 | **1387** | **+3.9%** | +8.1% |
| 13312 | BN=256 | 1399 | 1404 | +0.4% | +2.4% |
| 14336 | BN=256 | 1399 | 1404 | +0.4% | +1.9% |
| 20480 | BN=256 | 1427 | 1431 | +0.3% | +0.9% |
| 15360 | BN=256 | 1420 | 1421 | +0.1% | +1.7% |
| 19456 | BN=128 | 1420 | 1420 | 0.0% | +1.3% |
| 11264 | BN=128 | 1382 | 1378 | -0.3% | +3.6% |
| 17408 | BN=256 | 1413 | 1408 | -0.4% | +0.9% |
| 12288 | BN=128 | 1412 | 1405 | -0.5% | +0.6% |

几点观察：

* **只有 7168 真正吃到了收益**，从 1335 涨到 1387，即从 cuBLAS 的 95.3% 提到 99.0%。这正是 wave 效率最差的那个尺寸，也是模型给出的预测最大的那个（+8.1%）。
* 其余 8 个尺寸都在 -0.5% 到 +0.4% 之间，基本就是噪声。
* 实测普遍比模型少 1 到 4 个百分点。原因是模型没有算两件事：第二次 launch 要重新填一遍自己的流水线，而且由于 stream 上的顺序依赖，它必须等主体 kernel 完全排空才能开始 —— 前一个 wave 的 drain 就这么暴露出来了。**这个固定开销大约值 1 到 4 个点，所以只有模型预测超过它的切分才划得来。**
* 换句话说，这个方法的适用面比模型看上去要窄得多：它只在 wave 效率差得离谱、而且这个差距无法靠调 BK/GSM 补回来的尺寸上有意义。在这 18 个方阵里，符合条件的只有 7168 一个。

顺带一提，既然窄 tile 的 wave 效率更高，为什么不干脆全用 BN=128？因为算术强度会掉下来：BN=128 是 85.3 FLOP/byte，BN=512 是 170.7，正好差一倍。实测下来，把整个矩阵都换成 BN=128，即使 wave 效率从 86.5% 提到 98.8%，性能反而低 5.4% —— 每个 tile 本身慢了 11% 到 14%。所以窄 tile 只能用在尾波上：那里占的比重小，粒度才是主要矛盾；主体部分占的比重大，算术强度才是主要矛盾。这两个诉求本来就是矛盾的，而它们恰好作用在执行的不同阶段，这也正是"同一个 grid 上不必只有一个 BN"的理由。
