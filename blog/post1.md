...
## 第二种设计：BN512 基础版
对于 BN512 的情况，本质上就是在计算的时候，把完整的 TMEM 全部用上 —— 即两个 MMA buffer 同时被使用，以此提高计算效率，算得更高效一些。但是 trade off 在于每一轮算完以后，最后 Epilogue 开始的时候会稍微卡一下，直到 MMA buffer 被释放为止。在基础版中，我们不讨论两个 MMA buffer 的分开释放，而总是一次性释放两个，也就是两个都可以释放的时候再发出释放信号。
