...
BK=64，同样扫了 GSM 8/12/16 三档。

![BN=512 两轮轮转性能对比](https://raw.githubusercontent.com/tongzhou8086/mmc/main/blog/figures/perf-bn512-2round.png)

需要说明的是，这一组数字所在的节点比前几节慢一些 —— cuBLAS 这一列比前几节低了约 3.5%，所以**不要把这张图里的绝对值和前几节直接比**。为了能够直接比较，我们把第三种设计放在同一次运行、同一进程里交错打乱重测了一遍，两者各取最优 GSM 的对比如下：
...
* **18 个尺寸全部为正**，平均 +1.1%，最大 +3.0%，最小的一档也只是持平。逐格配对测量下没有一处倒退，这比平均值本身更说明问题。
* 收益的来源可以直接从设计上读出来：第三种设计的 MMA 循环是「panel 0 一个 k tile、panel 1 一个 k tile」交替，所以在 k=0 处 issue 完 panel 0 的一个 k tile 就要等 panel 1 的 accumulator，能用来掩盖 panel 1 draining 的工作只有 1 个 k tile；而两轮轮转的批量是 6，panel 0 一次能连发 6 个 k tile。换句话说，NS 在这里同时兼了两个职：既是 ring 的深度（对付 RAW 停顿），也是掩盖第二个 panel draining 的本钱（对付 WAR 停顿）。
* GSM 的规律和前几种设计一致：大尺寸上 GSM=16 最好，中间段 GSM=12 略优。
* 完整数字（含同一次运行的第三种设计对照表）见 [blog/perf-data.md](./perf-data.md)。
