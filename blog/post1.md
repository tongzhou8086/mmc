...
和 BN256 是同样的形状，但交叉点来得更早、幅度也大得多：从 8192 开始 BK=128 就已经反超，之后一路拉开，到 18432 上快出 **18.6%**。原因能对上前面的分析：BK 翻倍以后 k 迭代次数减半，每个 output tile 的 MMA 段落变长，而 BN512 只有一个 accumulator，draining 必须在下一个 output tile 开始之前完成，MMA 段落越长，这个时间窗口就越宽裕。BK=64 那一列从 6144 上的 1368 一路掉到 18432 上的 1154，正是这个窗口不够用时卡顿的直接体现。BN512 的窗口本来就比 BN256 紧（BN256 还可以拖到下下个 output tile），所以它对 BK 的敏感度也更高 —— 这也解释了为什么同样是加大 BK，BN512 这边的收益要大得多。

关于和 cuBLAS 的对比需要多说一句：上表中 torch.matmul 在多数尺寸上仍然领先，但这个结论对机器状态相当敏感。我们在不同的 B200 节点上重复测过，torch.matmul 在同一个尺寸上的读数可以相差 5% 上下，而 BK=128 那一列要稳定得多。上表中 BK=128 在 10240、14336 和 18432 上都是领先 cuBLAS 的（18432 上领先 2.1%），但换一个节点，结论就可能翻过去。上表取自同一个节点、三轮独立测量的中位数，每格的三轮离散度基本在 2% 以内，但跨节点的差异要大于这个数，所以几个百分点以内的胜负不宜当作定论。
