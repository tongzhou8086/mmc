...
本文会首先介绍一下矩阵乘法、算术强度的概念以及必要的 GPU 架构背景和 2-CTA MMA、CTA swizzling 这两种常规优化，熟悉这部分内容的读者可以自行跳过。这些都属于背景，背景之后，我们会先进行流水线设计的理论分析篇，从“数据流动”的角度来看待流水线的编排，我们总结出了 4 种 Buffer 和 5 种操作，每一种操作都可以认为是数据从一种 Buffer 流出，最后流进了另一种 Buffer。相当于是操作数放在源 Buffer 中，操作结果放在目的 Buffer 中。以“4 种 Buffer、5 种操作”为根基，我们又自然地推导出了 Buffer 的读写互斥性，即任何一个 Buffer 都无法同时被一个操作读和被另一个操作写，由此又进一步推导出了这 5 种操作之间任意两种要能够同时进行（即重叠起来）的条件。这便构成了流水线同步机制的根本原理。之后我们以单 buffer 为例，阐述流水线的时间线，从而看出两类会导致流水线上 MMA issue 卡顿的原因，然后分别给出对应的解决方案。

最后第三章是实现篇，实现篇会直接继承理论分析的结果，将它们转化成对应的 CUDA Kernel（文中是以伪代码的形式呈现）。作为例子，我们会呈现三种不同的设计方案，三种设计方案其实都是围绕着下面这个问题：一次 MMA 指令 issue 能产生多少计算量？以及 MMA 指令的 issue 能够多么无卡顿？我们流水线设计的最终目标则是这两指标的叠加，既想要无卡顿地 issue MMA 指令，同时也希望 MMA 指令一次 issue 出去能够产生更多的计算量。当然从这三种设计中，我们也可以看出，这两个指标常常是互相矛盾的，于是就需要取舍与权衡，或者通过一些更加精巧的设计来平衡。
