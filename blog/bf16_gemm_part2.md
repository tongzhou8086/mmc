...
### 流水线调度的参数配置
warp 配置好了以后，我们再来总结下流水线的参数配置以及一些计算方法。任何一种调度方式，至少都需要配置这两组参数：

* Tile sizes：BM / BN / BK 设为多少
* Buffer 配置：上述 4 种 Buffer 每种配几个、每个多大

先说 Buffer。如前文所述，同一类型的 Buffer 配置多份，目的是提高流水线的重叠程度，从而减少上面那两类停顿。但硬件资源（SMEM、TMEM、RMEM 的容量）是给定的，于是每种 Buffer 配多大、配几个，本身就成了一个设计问题 —— 这也是后文各个设计方案主要的着力点。

再说 Tile sizes。BM、BN、BK 三者的自由度并不一样：

* BM 固定为 128，与 TMEM 的 layout（128 行）一致，没有别的选择。M 无法被 128 整除时，需要做 out of bound 处理。
* BN 最大为 512。单次 MMA 操作支持的 N 可以是 64、128 或 256；而 TMEM 的大小是 128 行 x 512 列，这就把 BN 的上限压在了 512。后文会讨论 BN = 256 和 512 两种配置，它们达成的流水线状态相当不一样。
* BK 取 64 的整数倍。BK 是 A tile 的 inner dimension，于是它直接关系到内存访问是否连续：对 GPU 内存的读写，最好以 128 个连续字节为单位进行，才不浪费内存带宽 —— 而对 BF16 而言，64 个元素恰好就是 128 字节。

还要提前说明一点：本文所探讨的所有设计都默认开启 2 CTA MMA，因为它会同时影响 tile sizes 和各类 Buffer 的配置数量。开启之后，每个 CTA 只需从内存中读取它实际所需 BN 的一半 —— 这也是 2 CTA MMA 的另一重功效：除了提高算术强度以外，它还把每个 CTA 读取的 B tile 砍半，从而腾出更多 SMEM 空间来多配几个 TMA buffer 或者 Store buffer。

于是，以 BF16 计算，一个 A tile 占 `BM x BK x 2` 字节，一个 B tile 占 `BK x BN x 2 / 2` 字节 —— 末尾那个除以 2 就来自 2 CTA MMA。后文介绍具体的流水线配置时，我们都会用这两个式子来算一个 TMA buffer 该配多大。

流水线的理论部分至此介绍完毕，下一部分中我们开始看具体的调度设计。
