...
RAW 停顿来源于 read-after-write 数据依赖，减少停顿的方法则是预取（prefetch）数据，即 read 的时候同时开始下一轮的 write，这样可以减少下次 read 的时候的等待。这样的数据预取需要我们使用多个 TMA buffer，即当一个 MMA 开始时，与此同时，TMA load 可以同时开始往另一个 buffer 里面写入后续要被 MMA 消费的数据。
...
## 实现
在实现部分，我们会先看一下流水线调度的基本配置，譬如 BM、BN、BK 的大小等等，然后我们会探讨两种具体的设计方案。

### 各类 buffer 大小的计算方式
前面我们说过，有可能的话，我们会希望把 BM 和 BN 配置得更大，来提高算术强度。但 GPU 片上存储空间毕竟有限，BM 和 BN 不可能无限大。在计算各类 buffer 的大小之前，我们先把 Blackwell 的硬件限制集中列一下：
...
由这几条可以直接推出 tile sizes 的取值范围。BM 没有选择，只能是 128。BN 最大为 512，因为 TMEM 一共 512 列；当 BN 配为 256 时，一个 MMA buffer 是 128 行 x 256 列，整个 TMEM 正好放得下两个。BK 的配置则和内存访问的连续性有关：BK 是 A tile 的 inner dimension，在 BF16 下把 BK 配成 64 的倍数，一次访问正好凑满 128 个连续字节，不浪费任何内存带宽。
