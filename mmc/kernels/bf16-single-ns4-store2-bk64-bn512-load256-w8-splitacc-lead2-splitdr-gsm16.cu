...
// it keeps accumulating into panel 0 for LEAD_K=2 k-tiles, then replays the same
...
//
// This variant also splits the data-ready signal per 256-column MMA panel.
// Panel 0's data-ready fires as soon as panel 0 has taken its last k-tile -
// inside the lead block when the whole tile fits in the lead, otherwise in the
// steady-state arm - so the epilogue starts draining panel 0 while panel 1 is
// still accumulating. In the lead block it fires before the wait on panel 1's
// free barrier, so the drain can start while this warp is still blocked there.
//
// Adapted from bf16-single-ns4-store2-bk64-bn512-load256-w8-splitacc-lead2.cu,
...
//
// GROUP_SIZE_M=16 variant of bf16-single-ns4-store2-bk64-bn512-load256-w8-splitacc-lead2-splitdr.cu. Only the CTA-swizzle chunk
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                while (k < num_k) {
                    if (k == 0) {
                        ...
                        // If the lead covered every k-tile there is, panel 0
                        // is finished: signal its data-ready here, ahead of the
                        // wait below, so the epilogue can start draining panel 0
                        // while this warp is still blocked on panel 1.
                        if (lead == num_k)
                        ...
                        issue_mma_chain(d_tmem,
                                        ...
                                        idesc,
                                        /*first_k_tile=*/ false);
                        // Panel 0 has now taken its last k-tile. tcgen05.commit
                        // signals on completion of what was issued before it,
                        // so this covers panel 0 exactly.
                        if (k == num_k - 1)
                            signal_on_mma_completion(
                                (uint32_t)__cvta_generic_to_shared(
                                    &mbar_tmem_data_ready[0]), cta_mask);
