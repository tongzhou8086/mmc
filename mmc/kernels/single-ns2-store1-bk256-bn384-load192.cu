...
// Single-TMEM BN=384 BK=256 variant, for higher arithmetic intensity per
// output tile. Only NS=2 slots fit in shared memory at BK=256 and BN=384.
//
// A tcgen05 MMA takes N <= 256, so each K step issues an N=256 MMA followed by
// an N=128 MMA into the next 128 accumulator columns. With cta_group::2 every
// MMA splits N across the CTA pair, so per CTA the B tile is 128 rows for the
// N=256 MMA plus 64 rows for the N=128 MMA, and those two pieces come from
// different global N offsets:
//
//   CTA r holds B rows [base_n + r*128, +128) and [base_n + 256 + r*64, +64).
//
// The 64-row piece needs its own TMA descriptor, hence the extra B_tail_tmap
// kernel argument. Accumulator columns still map to global N contiguously:
// the N=256 MMA covers [base_n, +256) and the N=128 MMA covers [base_n+256,
// +384), so the epilogue just drains six 64-column store tiles in order.
//
// The 384-column accumulator does not fit in registers for all eight epilogue
// warps at once (384*128/256 threads = 192 floats/thread, above the 168-register
// budget at 384 launch threads), so the epilogue loads and drains it in two
// 192-column halves and releases TMEM after the second load.
...
template <
    ...
    typename BTailScaleTensor0,
    typename AScaleTensor1,
    typename BScaleTensor1,
    typename BTailScaleTensor1>
__device__ __forceinline__ void issue_mxfp8_mma2_abt_128x384x256(
    ...
    const BTailScaleTensor0& b_tail_scale_0,
    const AScaleTensor1& a_scale_1,
    const BScaleTensor1& b_scale_1,
    const BTailScaleTensor1& b_tail_scale_1,
    uint32_t smem_buffer_free_mbarrier_addr
) {
    static_assert(BM == 128 && BN == 384 && BK == 256);
    ...
    issue_mxfp8_mma2_abt_chunk<Accumulate>(
        ...
        a_scale_0.addr, b_scale_0.addr, main_idescs[0]);
    ...
    for (int sfid = 1; sfid < 4; ++sfid) {
        issue_mxfp8_mma2_abt_chunk<1>(
            ...
            a_scale_0.addr, b_scale_0.addr, main_idescs[sfid]);
    }
    ...
    for (int sfid = 0; sfid < 4; ++sfid) {
        issue_mxfp8_mma2_abt_chunk<1>(
            ...
            a_scale_1.addr, b_scale_1.addr, main_idescs[sfid]);
    }
    ...
    issue_mxfp8_mma2_abt_chunk<Accumulate>(
        ...
        a_scale_0.addr, b_tail_scale_0.addr, tail_idescs[0]);
    ...
    for (int sfid = 1; sfid < 4; ++sfid) {
        issue_mxfp8_mma2_abt_chunk<1>(
            ...
            a_scale_0.addr, b_tail_scale_0.addr, tail_idescs[sfid]);
    }
    ...
    for (int sfid = 0; sfid < 4; ++sfid) {
        issue_mxfp8_mma2_abt_chunk<1>(
            tail_dst,
            k_major_operand_chunk_desc<a_tile>(a_base, 4 + sfid),
            k_major_operand_chunk_desc<b_tail_tile>(b_tail_base, 4 + sfid),
            a_scale_1.addr, b_tail_scale_1.addr, tail_idescs[sfid]);
    }
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    ScaleAtom (&b_sc_smem)[NS][N_SCALE_ATOMS][K_SCALE_ATOMS] =
        al.allocate<ScaleAtom[NS][N_SCALE_ATOMS][K_SCALE_ATOMS]>();
    ...
    constexpr int A_SCALE_LOGICAL_COLS_PER_SLOT = 16 * K_SCALE_ATOMS;
    constexpr int B_SCALE_LOGICAL_COLS_PER_SLOT =
        16 * N_SCALE_ATOMS * K_SCALE_ATOMS;
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                ...
                for (int k_half = 0; k_half < K_SCALE_ATOMS; ++k_half) {
                    ...
                    // Every CTA of the pair runs tcgen05.cp against its own
                    // SMEM, so each one needs all N scale atoms locally.
                    #pragma unroll
                    for (int atom = 0; atom < N_SCALE_ATOMS; ++atom) {
                        load_scale_atom_from_gmem_to_smem(
                            b_sc_smem[slot][atom][k_half],
                            &B_sc_tmap,
                            base_n / 128 + atom, scale_k,
                            compute_data_ready_cta0,
                            static_cast<uint16_t>(1 << cta_rank));
                    }
                }
                ...
                constexpr int slot_bytes =
                    ...
                    sizeof(ScaleAtom) * K_SCALE_ATOMS * (1 + N_SCALE_ATOMS);
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                int slot = gk % NS;
                constexpr int A_SCALE_COLS_PER_SLOT =
                    A_SCALE_LOGICAL_COLS_PER_SLOT;
                constexpr int B_SCALE_COLS_PER_SLOT =
                    B_SCALE_LOGICAL_COLS_PER_SLOT;
                // Per slot the B scale atoms are laid out K-half major, so the
                // N=256 MMA of each K half reads a contiguous 2-atom block and
                // the N=128 MMA reads the third atom right after it.
                constexpr int B_SCALE_COLS_PER_K_HALF = 16 * N_SCALE_ATOMS;
                ...
                auto b_sc_atom_01 = b_sc_tm.template subtile<16>(
                    b_scale_slot_col + B_SCALE_COLS_PER_K_HALF);
                auto b_sc_atom_11 = b_sc_tm.template subtile<16>(
                    b_scale_slot_col + B_SCALE_COLS_PER_K_HALF + 16);
                auto b_sc_atom_21 = b_sc_tm.template subtile<16>(
                    b_scale_slot_col + B_SCALE_COLS_PER_K_HALF + 32);
                auto b_sc_stage_0 =
                    b_sc_tm.template subtile<32>(b_scale_slot_col);
                auto b_sc_stage_1 = b_sc_tm.template subtile<32>(
                    b_scale_slot_col + B_SCALE_COLS_PER_K_HALF);
                auto b_sc_tail_stage_0 = b_sc_atom_20;
                auto b_sc_tail_stage_1 = b_sc_atom_21;
                ...
                copy_scale_atom_from_smem_to_tmem(
                    b_sc_atom_10.addr, b_sc_smem[slot][1][0]);
                copy_scale_atom_from_smem_to_tmem(
                    b_sc_atom_20.addr, b_sc_smem[slot][2][0]);
                copy_scale_atom_from_smem_to_tmem(
                    b_sc_atom_01.addr, b_sc_smem[slot][0][1]);
                copy_scale_atom_from_smem_to_tmem(
                    b_sc_atom_11.addr, b_sc_smem[slot][1][1]);
                copy_scale_atom_from_smem_to_tmem(
                    b_sc_atom_21.addr, b_sc_smem[slot][2][1]);

                if (k == 0) {
                    issue_mxfp8_mma2_abt_128x384x256<0>(
                        out_tm, a_smem[slot], b_smem[slot], b_tail_smem[slot],
                        a_sc_stage_0, b_sc_stage_0, b_sc_tail_stage_0,
                        a_sc_stage_1, b_sc_stage_1, b_sc_tail_stage_1,
                        compute_buffer_free_addr);
                } else {
                    issue_mxfp8_mma2_abt_128x384x256<1>(
                        out_tm, a_smem[slot], b_smem[slot], b_tail_smem[slot],
                        a_sc_stage_0, b_sc_stage_0, b_sc_tail_stage_0,
                        a_sc_stage_1, b_sc_stage_1, b_sc_tail_stage_1,
                        compute_buffer_free_addr);
