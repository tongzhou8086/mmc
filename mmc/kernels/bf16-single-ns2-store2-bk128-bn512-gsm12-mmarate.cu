...
//
// Imported from mxfp8-gemm-study/bn512-single-tmem/kernel.cu. It does not yet
...
//
// BK=128 variant of bf16-single-ns4-store2-bk64-bn512.cu. Doubling BK doubles the bytes per
...
// ── MMA issue-rate instrumentation ──────────────────────────────────
// Two timestamps per output tile, taken in the MMA warp: one immediately
// before that tile's first MMA issue, one immediately after its last. That
// splits the tile into the two phases the throughput model needs -
//
//     compute_i = t_end(i)     - t_begin(i)     the k loop
//     pause_i   = t_begin(i+1) - t_end(i)       everything between tiles
//
// - and since the compute produced per tile is exact arithmetic, the issue
// rate inside the k loop comes out as a measurement rather than a fit.
// Layout per cluster: [count, t_base, then 2 u64 per tile]. nullptr disables.
constexpr int RATE_MAX_TILES = 4096;
constexpr int RATE_SLICE_U64 = 2 + 2 * RATE_MAX_TILES;

__device__ __forceinline__ uint64_t rate_clock() {
    uint64_t t;
...
__device__ __forceinline__ void matmul_cluster_impl(
...

    {
        ...
        if (warp_id == 0 && elect_sync()) {
            ...
            uint64_t* rate = (rate_buffer == nullptr)
                           ? nullptr
                           : rate_buffer + (size_t)cluster_pid * RATE_SLICE_U64;
            if (rate != nullptr) { rate[0] = 0; rate[1] = rate_clock(); }
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                for (int k = 0; k < num_k; k++) {
                    ...
                    if (rate != nullptr && k == 0) t_begin = rate_clock();
                ...
                if (rate != nullptr) {
                    const uint64_t n = rate[0];
                    if (n < (uint64_t)RATE_MAX_TILES) {
                        rate[2 + 2 * n]     = t_begin;
                        rate[2 + 2 * n + 1] = rate_clock();
                        rate[0] = n + 1;
                    }
                }
