...
// The epilogue drains the upper 192 accumulator columns first, which covers all
// of the N=128 group's columns [BN_MAIN, BN), releases the N=128 barrier, and
// only then drains the lower 192 columns and releases the N=256 barrier. The MMA
// warp waits on them separately and issues the N=128 group first, so the next
// output tile's N=128 MMA overlaps the rest of this tile's epilogue.
//
// The drain order matters: the 192-column halves are [0,192) and [192,384) while
// the N=128 columns are [256,384), so only the upper half can be freed first.
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            // The upper register load must cover every N=128 accumulator column
            // for the early release of that region to be legal.
            static_assert(NUM_LOADS == 2 && LOAD_N >= BN_TAIL);
            static_assert(
                LOADS_PER_WARP * COL_GROUPS == LOADS_PER_STORE_TILE);

            #pragma unroll
            for (int load = 0; load < NUM_LOADS; load++) {
                // Upper half first: it holds the N=128 accumulator columns, so
                // that region can be released after the first register load.
                const int load_base = (NUM_LOADS - 1 - load) * LOAD_N;
                ...
                for (int tile = 0; tile < STORE_TILES_PER_LOAD; tile++) {
                    ...
                    for (int n = 0; n < LOADS_PER_WARP; n++) {
                        ...
                        const int column = load_base + tile * STORE_N +
                            local_n * LOAD_WIDTH;
