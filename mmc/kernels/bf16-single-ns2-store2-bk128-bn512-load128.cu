...
//
// This variant drains the accumulator with a two-level epilogue loop: the outer
// loop pulls 128 columns out of TMEM at once (4 x tcgen05.ld.32x32b.x32), the
// inner loop hands them to the TMA store 64 columns at a time. That is 4 TMEM->
// reg trips per output tile instead of 8, at the cost of holding 128 floats per
// lane instead of 64.
//
// Adapted from bf16-single-ns4-store2-bk64-bn512.cu, which is otherwise
...
//
// BK=128 variant of bf16-single-ns4-store2-bk64-bn512-load128.cu. Doubling BK doubles the bytes per
// ring slot, so the ring drops from 4 slots to 2 to keep the same 192 KB of
// staging: fewer, larger loads and half as many k iterations, at the cost of
// half the run-ahead depth. Requires K % 128 == 0.
//
// A is fetched as two 64-element K chunks, because 128B swizzle caps a TMA
// box at 64 BF16 elements in the contiguous dimension.
//
// Launch contract, mirrored by Runtime.launch_bf16:
//   grid    = sm_count - sm_count % CTA_GROUP   (persistent, EPILOGUE_OVERLAP)
//   block   = LAUNCH_THREADS = (NUM_WARPS + 4) * 32 = 256
...

    {
        ...
        if (warp_id == 0 && elect_sync()) {
            ...
            for (int b = 0; b < 2; b++) {
                ...
                mbarrier_arrive_no_tx_cluster_cta0(mbar_tmem_buffer_free[b]);
            }
...
#define EPI_OUT_COL_BASE            base_n
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                constexpr int LDW = TCGEN05_LD_WIDTH;
                {
                    ...
                    // stores them STORE_N columns at a time.
                    constexpr int LOAD_N          = 128;                 // columns per outer load
                    constexpr int LD_REGS         = 32;                  // floats per tcgen05.ld.x32
                    constexpr int NUM_LOADS       = BN / LOAD_N;         // 4 TMEM->reg trips
                    constexpr int CHUNKS_PER_LOAD = LOAD_N / STORE_N;    // 2 stores per trip
                    constexpr int LDS_PER_CHUNK   = STORE_N / LD_REGS;   // 2 loads per store
                    constexpr int LDS_PER_LOAD    = LOAD_N / LD_REGS;    // 4
                    constexpr int PACKS_PER_LD    = LD_REGS / 8;         // int4 writes per load
                    static_assert(STORE_N == 64, "pipelined TMA store assumes STORE_N=64");
                    static_assert(COL_GROUPS == 1, "two-level drain assumes one column group");
                    static_assert(NUM_LOADS * LOAD_N == BN, "BN must divide into LOAD_N groups");
                    static_assert(LDS_PER_CHUNK * CHUNKS_PER_LOAD == LDS_PER_LOAD, "");
                    int store_stage = 0;

                    #pragma unroll
                    for (int load = 0; load < NUM_LOADS; load++) {
                        float t[LDS_PER_LOAD][LD_REGS];
                        #pragma unroll
                        for (int j = 0; j < LDS_PER_LOAD; j++)
                            tcgen05_ld_32x32b_x32(
                                trow + (uint32_t)(load * LOAD_N + j * LD_REGS), t[j]);
                        tcgen05_wait_ld();
                        ...
                        // free-store-slot wait, not after it. With the wider
                        // loads this lands one store earlier than in the
                        // 64-column version.
                        //
                        ...
                        for (int c = 0; c < CHUNKS_PER_LOAD; c++) {
                            ...
                            for (int j = 0; j < LDS_PER_CHUNK; j++) {
                                const int jj = c * LDS_PER_CHUNK + j;
                                #pragma unroll
                                for (int g = 0; g < PACKS_PER_LD; g++) {
                                    ...
                                    for (int i = 0; i < 4; i++)
                                        pk[i] = __floats2bfloat162_rn(
                                            mmc_epi(t[jj][g * 8 + 2 * i]),
                                            mmc_epi(t[jj][g * 8 + 2 * i + 1]));
                                    const int local_n = j * PACKS_PER_LD + g;
