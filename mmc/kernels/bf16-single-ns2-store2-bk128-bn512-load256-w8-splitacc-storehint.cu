...
// C is written once and never read again by this kernel, so its lines are
// pure L2 pollution: they compete with the A/B panels that neighbouring CTAs
// are about to reuse, which is exactly the working set GSM exists to protect.
// Tagging the store evict_first makes those lines the first eviction
// candidates. This is the only difference from the parent kernel.
...
__device__ __forceinline__ void tma_2d_store(
    ...
    asm volatile(
        ...
        "[%0, {%1, %2}], [%3], %4;"
        :: "l"(tmap), "r"(x), "r"(y), "r"(smem_src),
           "l"(l2_evict_first_policy()) : "memory");
}
