...
// This variant lets the MMA warp start the next output tile's scale copies
// before the epilogue has released the accumulator buffer. The scale atoms live
// in their own TMEM columns, so only the MMA itself depends on the accumulator
// buffer being free; the tcgen05.cp copies do not.
