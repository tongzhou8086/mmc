...
// BF16 GEMM: C[M,N] = A[M,K] @ B[K,N], double-TMEM-buffered with a 2-CTA
// cluster MMA and a chunked TMA-store epilogue. Unlike the MXFP8 kernels this
// is an AB (not ABt) kernel: B is conventional row-major [K,N], and the TMA
// descriptors are rank 2 rather than rank 5.
//
// Imported from mxfp8-gemm-study/bn256-double-tmem/kernel.cu. It does not yet
// use cuda-mxfp8.cuh; the data-type-agnostic helpers will be split out of that
// header and shared with this kernel separately.
//
...
// and requires M % (CTA_GROUP * BM) == 0, N % BN == 0, K % BK == 0.
//
