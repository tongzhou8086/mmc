...
//
// BK=128 variant of bf16-double-ns6-store2-bk64.cu. Doubling BK doubles the
// bytes per ring slot, so the ring drops from 6 slots to 3 to keep the same
// 192 KB of staging: fewer, larger loads and half as many k iterations, at
// the cost of half the run-ahead depth. Requires K % 128 == 0.
//
...
constexpr int B_SLOT_BYTES = BN_LOCAL * BK * BF16_BYTES;       // 32 KB
constexpr int SLOT_BYTES   = A_SLOT_BYTES + B_SLOT_BYTES;      // 64 KB / slot
