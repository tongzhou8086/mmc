...
//
// Adapted from bf16-single-ns4-store2-bk64-bn512-load128.cu, which is otherwise
...
//
// BK=128 variant of bf16-single-ns4-store2-bk64-bn512-load256-w8.cu. Doubling BK
// doubles the bytes per ring slot, so the ring drops from 4 slots to 2 to keep
// the same 192 KB of staging: fewer, larger loads and half as many k iterations,
// at the cost of half the run-ahead depth. Requires K % 128 == 0.
//
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                constexpr int LDW = TCGEN05_LD_WIDTH;
                {
                    ...
                    for (int load = 0; load < NUM_LOADS; load++) {
                        ...
                        // free-store-slot wait, not after it. With two trips
                        // this lands with half the tile still to store.
                        //
