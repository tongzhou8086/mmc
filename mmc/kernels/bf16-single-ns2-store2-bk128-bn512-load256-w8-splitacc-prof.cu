...
// ── MMA-issue wait profiling ────────────────────────────────────────
// The MMA warp blocks on exactly two things: the TMA buffer it is about to
// read becoming readable (data-ready), and the accumulator panel it is about
// to write becoming writable (buffer-free). Those are the two conditions the
// data-flow model says an operation needs, so recording them is recording the
// pipeline's stalls directly.
//
// A wait that does not block is not recorded and costs one non-blocking
// mbarrier.test_wait. Only when the test fails do we read the clock, block,
// and read it again - so the instrumentation cannot manufacture the stalls it
// is measuring.
//
// Layout, one slice per cluster: [count, t_base, then 4 u64 per event].
constexpr int PROF_MAX_EVENTS = 8192;
constexpr int PROF_SLICE_U64  = 2 + 4 * PROF_MAX_EVENTS;

// event kinds
constexpr uint32_t PROF_DATA_READY = 0;   // waiting for a TMA buffer to fill
constexpr uint32_t PROF_PANEL_FREE = 1;   // waiting for an accumulator panel

__device__ __forceinline__ uint64_t prof_clock() {
    uint64_t t;
    asm volatile("mov.u64 %0, %%clock64;" : "=l"(t));
    return t;
}

// non-blocking probe: is this barrier already satisfied?
__device__ __forceinline__ bool barrier_ready(uint32_t mb, uint32_t phase) {
    uint32_t ok;
    asm volatile(
        "{\n\t .reg .pred P;\n\t"
        "mbarrier.test_wait.parity.shared::cta.b64 P, [%1], %2;\n\t"
        "selp.u32 %0, 1, 0, P;\n\t }"
        : "=r"(ok) : "r"(mb), "r"(phase) : "memory");
    return ok != 0;
}

__device__ __forceinline__ void wait_phase_prof(
    uint32_t mb, uint32_t phase, uint64_t* slice,
    uint32_t kind, uint32_t detail, uint32_t ti, uint32_t k)
{
    if (slice == nullptr) { wait_phase(mb, phase); return; }
    if (barrier_ready(mb, phase)) { wait_phase(mb, phase); return; }
    const uint64_t t0 = prof_clock();
    wait_phase(mb, phase);
    const uint64_t t1 = prof_clock();
    const uint64_t n = slice[0];
    if (n < (uint64_t)PROF_MAX_EVENTS) {
        uint64_t* e = slice + 2 + 4 * n;
        e[0] = ((uint64_t)kind << 48) | ((uint64_t)detail << 40)
             | ((uint64_t)ti << 16) | (uint64_t)k;
        e[1] = t0;
        e[2] = t1;
        e[3] = 0;
        slice[0] = n + 1;
...
__device__ __forceinline__ void matmul_cluster_impl(
...

    {
        ...
        if (warp_id == 0 && elect_sync()) {
            ...
            long gk = 0;
            // one slice per cluster; the MMA warp is a single elected thread,
            // so nothing here needs atomics
            uint64_t* prof = (prof_buffer == nullptr)
                           ? nullptr
                           : prof_buffer + (size_t)cluster_pid * PROF_SLICE_U64;
            if (prof != nullptr) { prof[0] = 0; prof[1] = prof_clock(); }
            for (int ti = 0; ti < num_my; ti++) {
                const int buf = 0;
                ...
                for (int k = 0; k < num_k; k++) {
                    ...
                    uint32_t compute_buffer_free_addr =
                        (uint32_t)__cvta_generic_to_shared(&mbar_compute_buffer_free[slot]);
                    wait_phase_prof(compute_data_ready_addr,
                                    compute_data_ready_phase[slot], prof,
                                    PROF_DATA_READY, (uint32_t)slot,
                                    (uint32_t)ti, (uint32_t)k);
                    if (k == 0) {
                        ...
                        wait_phase_prof(
                            ...
                            tmem_panel_free_phase[0], prof,
                            PROF_PANEL_FREE, 0, (uint32_t)ti, (uint32_t)k);
                        ...
                        wait_phase_prof(
                            ...
                            tmem_panel_free_phase[1], prof,
                            PROF_PANEL_FREE, 1, (uint32_t)ti, (uint32_t)k);
            ...
            if (prof != nullptr) {
                // t_end goes in the unused word of a sentinel event, so the
                // reader knows the full span the timeline covers
                const uint64_t n = prof[0];
                if (n < (uint64_t)PROF_MAX_EVENTS) {
                    uint64_t* e = prof + 2 + 4 * n;
                    e[0] = (uint64_t)0xFFFF << 48;   // sentinel
                    e[1] = prof_clock();
                    e[2] = e[1];
                    e[3] = 0;
                    prof[0] = n + 1;
...
void matmul_cluster(
    ...
    uint64_t* prof_buffer          // nullptr disables all instrumentation
)
{
