...
//
// GROUP_SIZE_M=12 variant of bf16-single-ns4-store2-bk64-bn512-load256-w8-splitacc-splitdr.cu. Only the CTA-swizzle chunk
                ...
                for (int k = 0; k < num_k; k++) {
                    ...
                    if (k == 0) {
                        ...
                        issue_mma_chain(d_tmem,
                                        ...
                                        idesc,
                                        /*first_k_tile=*/ false);
                        // Panel 0 has now taken its last k-tile, so its
                        // data-ready fires here rather than after panel 1's
                        // MMAs: tcgen05.commit signals on completion of what
                        // was issued before it, so this covers panel 0 exactly.
                        if (k == num_k - 1)
                            signal_on_mma_completion(
                                (uint32_t)__cvta_generic_to_shared(
                                    &mbar_tmem_data_ready[0]), cta_mask);
