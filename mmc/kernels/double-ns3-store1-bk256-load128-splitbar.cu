...
// MXFP8 double-TMEM-buffering BK=256 variant. The epilogue loads 128
// accumulator columns into registers before draining two 64-column TMA store
// tiles. This frees the 72-column scale window after the first TMEM load.
// This variant splits the per-slot data-ready barrier into one barrier for the
// scale atoms and one for the operand tiles. The TMA warp issues the scale
// loads first and completes them into the scale barrier, so the MMA warp can
// copy the scale atoms into TMEM as soon as they land instead of waiting for
// the much larger operand tiles.
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    __shared__ uint64_t mbar_compute_scale_ready[NS];
    __shared__ uint64_t mbar_compute_operand_ready[NS];
    ...
    constexpr int TMEM_SCALE_WINDOW_COLS = 2 * STORE_N;
    constexpr int TMEM_E8M0_PER_PHYS_COL = 4;
    constexpr int TMEM_A_SCALE_LOGICAL_COLS = 16 * K_SCALE_ATOMS * NS;
    constexpr int TMEM_B_SCALE_LOGICAL_COLS = 32 * K_SCALE_ATOMS * NS;
    ...
    static_assert(TMEM_SCALE_WINDOW_COLS == 128);
    ...
    if (warp_id == 0 && elect_sync()) {
        #pragma unroll
        for (int s = 0; s < NS; s++) {
            mbarrier_init(mbar_compute_scale_ready[s], CTA_GROUP);
            mbarrier_init(mbar_compute_operand_ready[s], CTA_GROUP);
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                ...
                uint32_t compute_scale_ready_cta0 =
                    mbarrier_addr_in_cta(mbar_compute_scale_ready[slot], 0);
                uint32_t compute_operand_ready_cta0 =
                    mbarrier_addr_in_cta(mbar_compute_operand_ready[slot], 0);
                ...
                constexpr int operand_bytes = sizeof(a_tile) + sizeof(b_tile);
                mbarrier_arrive_expect_tx_cta0(
                    mbar_compute_operand_ready[slot], operand_bytes);
                compute_buffer_free_phase[slot] ^= 1;
                gk++;
            }
        }
    } else if (cta_rank == 0 && warp_id == 1 && elect_sync()) {
        uint32_t compute_scale_ready_phase[NS] = {};
        uint32_t compute_operand_ready_phase[NS] = {};
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                ...
                mbarrier_wait_phase(
                    mbar_compute_scale_ready[slot],
                    compute_scale_ready_phase[slot]);
                ...
                mbarrier_wait_phase(
                    mbar_compute_operand_ready[slot],
                    compute_operand_ready_phase[slot]);
                ...
                compute_scale_ready_phase[slot] ^= 1;
                compute_operand_ready_phase[slot] ^= 1;
                gk++;
            }
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            constexpr int LOAD_COLS = 128;
            constexpr int STORE_TILES_PER_LOAD = LOAD_COLS / STORE_N;
            ...
            constexpr int NUM_LOADS = BN / LOAD_COLS;
            static_assert(STORE_N == 64);
            static_assert(LOAD_COLS == 2 * STORE_N);
            static_assert(
            ...
            for (int load = 0; load < NUM_LOADS; load++) {
                ...
                for (int tile = 0; tile < STORE_TILES_PER_LOAD; tile++) {
                    ...
                    for (int n = 0; n < LOADS_PER_WARP; n++) {
                        ...
                        const int column =
                            load * LOAD_COLS + tile * STORE_N + local_n * LOAD_WIDTH;
                ...
                for (int tile = 0; tile < STORE_TILES_PER_LOAD; tile++) {
                    ...
                    if (ew == 0 && elect_sync()) {
                        ...
                        store_bf16_tile_from_smem_to_gmem(
                            &D_tmap, d_smem[store_stage], local_m,
