...
// This variant splits the accumulator-free barrier in two, one per MMA N group,
// and aligns the epilogue's register loads with those groups instead of using
// two equal halves:
//
//   load the 128 N=128-group columns [BN_MAIN, BN) -> release the N=128 barrier
//     -> stage and store those 2 tiles
//   load the 256 N=256-group columns [0, BN_MAIN)  -> release the N=256 barrier
//     -> stage and store those 4 tiles
//
// Each group's columns leave TMEM in a single load, so its barrier is released
// before any of that group's SMEM staging or TMA stores, and the next output
// tile's MMA for that group overlaps all of it. The 256-column load holds 128
// floats/thread, the same register footprint as the BN=256 -load256 epilogue,
// and the 128-column load that precedes it is dead by then, so the peak is
// unchanged from that known-good configuration.
...
template <int ScaleFactorId, int MmaN>
__device__ __forceinline__ constexpr uint32_t mxfp8_mma2_abt_256xN_desc() {
    static_assert(ScaleFactorId >= 0 && ScaleFactorId < 4);
    static_assert(MmaN > 0 && MmaN <= 256 && MmaN % 8 == 0);
    constexpr int mma_m = CTA_GROUP * BM;
    constexpr int mma_n = MmaN;
...
// The two N groups are issued separately so the MMA warp can wait on their
// accumulator-free barriers independently. Each writes disjoint accumulator
// columns, so each needs its own non-accumulating first chunk on the first K
// step. The N=256 group carries the tcgen05.commit, which covers every prior
// async tcgen05 operation from this warp - including the N=128 group and the
// scale copies - so the SMEM slot is released only once all of them retire.
template <
    int Accumulate,
    typename AccumTensor,
    typename AScaleTensor,
    typename BTailScaleTensor>
__device__ __forceinline__ void issue_mxfp8_mma2_abt_tail_n128(
    ...
    static_assert(CTA_GROUP == 2 && BN_LOCAL_TAIL == 64);

    constexpr uint32_t tail_idescs[4] = {
        mxfp8_mma2_abt_256xN_desc<0, BN_TAIL>(),
        mxfp8_mma2_abt_256xN_desc<1, BN_TAIL>(),
        mxfp8_mma2_abt_256xN_desc<2, BN_TAIL>(),
        mxfp8_mma2_abt_256xN_desc<3, BN_TAIL>(),
    };
...
__device__ __forceinline__ void issue_mxfp8_mma2_abt_main_n256(
    const AccumTensor& dst,
    const a_tile& a,
    const b_tile& b,
    const AScaleTensor& a_scale,
    ...
    constexpr uint32_t main_idescs[4] = {
        mxfp8_mma2_abt_256xN_desc<0, BN_MAIN>(),
        mxfp8_mma2_abt_256xN_desc<1, BN_MAIN>(),
        mxfp8_mma2_abt_256xN_desc<2, BN_MAIN>(),
        mxfp8_mma2_abt_256xN_desc<3, BN_MAIN>(),
    };
    const uint64_t a_base = make_k_major_operand_smem_desc(a);
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    __shared__ uint64_t mbar_tmem_accum_main_free;
    __shared__ uint64_t mbar_tmem_accum_tail_free;
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        mbarrier_init(mbar_tmem_accum_main_free, CTA_GROUP);
        mbarrier_init(mbar_tmem_accum_tail_free, CTA_GROUP);
        mbarrier_arrive_no_tx_cluster_cta0(mbar_tmem_accum_main_free);
        mbarrier_arrive_no_tx_cluster_cta0(mbar_tmem_accum_tail_free);
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        uint32_t tmem_accum_main_free_phase = 0;
        uint32_t tmem_accum_tail_free_phase = 0;
        long gk = 0;
        ...
        for (int ti = 0; ti < num_my; ti++) {
            for (int k = 0; k < num_k; k++) {
                ...
                if (k == 0) {
                    ...
                    // epilogue releases either accumulator region. The N=128
                    // columns are released first, so that group goes first.
                    mbarrier_wait_phase(
                        mbar_tmem_accum_tail_free, tmem_accum_tail_free_phase);
                    tmem_accum_tail_free_phase ^= 1;
                    tcgen05_fence_after_thread_sync();
                    issue_mxfp8_mma2_abt_tail_n128<0>(
                        out_tm, a_smem[slot], b_tail_smem[slot],
                        a_sc_stage, b_sc_tail_stage);

                    mbarrier_wait_phase(
                        mbar_tmem_accum_main_free, tmem_accum_main_free_phase);
                    tmem_accum_main_free_phase ^= 1;
                    tcgen05_fence_after_thread_sync();
                    issue_mxfp8_mma2_abt_main_n256<0>(
                        out_tm, a_smem[slot], b_smem[slot],
                        a_sc_stage, b_sc_stage, compute_buffer_free_addr);
                } else {
                    issue_mxfp8_mma2_abt_tail_n128<1>(
                        out_tm, a_smem[slot], b_tail_smem[slot],
                        a_sc_stage, b_sc_tail_stage);
                    issue_mxfp8_mma2_abt_main_n256<1>(
                        out_tm, a_smem[slot], b_smem[slot],
                        a_sc_stage, b_sc_stage, compute_buffer_free_addr);
                }
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            constexpr int TAIL_STORE_TILES = BN_TAIL / STORE_N;
            constexpr int MAIN_STORE_TILES = BN_MAIN / STORE_N;
            static_assert(STORE_N == 64);
            static_assert(COL_GROUPS == 2);
            static_assert(
                TAIL_STORE_TILES + MAIN_STORE_TILES == NUM_STORE_TILES);
            static_assert(
                LOADS_PER_WARP * COL_GROUPS == LOADS_PER_STORE_TILE);

            // One register load per MMA N group, the N=128 group first so its
            // barrier is released earliest. Each group's accumulator columns
            // are released as soon as they are in registers, before any of that
            // group's SMEM staging or TMA stores.
            //
            // The phase loop is unrolled over a constant trip count, so in each
            // copy `first_column` and `tiles` fold to compile-time constants and
            // the inner loops unroll exactly as if they were written out twice.
            constexpr int NUM_PHASES = 2;
            static_assert(MAIN_STORE_TILES >= TAIL_STORE_TILES);

            // Sized for the larger phase and shared by both, so the peak
            // register footprint is the 256-column load's 128 floats/thread -
            // the same as the BN=256 -load256 epilogue. The 128-column phase
            // leaves the tail of the array unused.
            float t[MAIN_STORE_TILES][LOADS_PER_WARP][LOAD_WIDTH];

            #pragma unroll
            for (int phase = 0; phase < NUM_PHASES; phase++) {
                const int first_column = (phase == 0) ? BN_MAIN : 0;
                const int tiles =
                    (phase == 0) ? TAIL_STORE_TILES : MAIN_STORE_TILES;
                ...
                for (int tile = 0; tile < tiles; tile++) {
                    ...
                    for (int n = 0; n < LOADS_PER_WARP; n++) {
                        ...
                        const int column = first_column + tile * STORE_N +
                            local_n * LOAD_WIDTH;
                ...
                if (ew == 0 && elect_sync()) {
                    if (phase == 0) {
                        mbarrier_arrive_no_tx_cluster_cta0(
                            mbar_tmem_accum_tail_free);
                    } else {
                        mbarrier_arrive_no_tx_cluster_cta0(
                            mbar_tmem_accum_main_free);
                    }
                }
