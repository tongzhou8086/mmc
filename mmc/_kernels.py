...
class KernelSpec:
    ...
    # each CTA of the pair holds 128 B rows for the N=256 MMA plus this many
    # for the N=128 MMA, from a different global N offset.
    bn_local_tail: int = 0
...
KERNELS = (
    ...
    # BN=384 variants: one N=256 MMA plus one N=128 MMA per K step, for higher
    # arithmetic intensity per output tile. The wider B tile and the third scale
    # atom per slot cost shared memory, so these run fewer pipeline stages.
    KernelSpec(
        "single-ns4-store1-bk128-bn384-load192", 128, 384, 226304,
        bn_local_tail=64,
    ),
    KernelSpec(
        "single-ns4-store3-bk128-bn384-load192", 128, 384, 224768,
        bn_local_tail=64,
    ),
    KernelSpec(
        "single-ns2-store1-bk256-bn384-load192", 256, 384, 226304,
        bn_local_tail=64,
    ),
