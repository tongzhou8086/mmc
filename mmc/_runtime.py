...
class Runtime:
    ...
    def _build_bf16_launch_args(self, spec, a, b, out, stream, prof=None):
        ...
        # instrumented kernels take a trailing profiling buffer; passing null
        # is what disables the instrumentation, so the same cubin can be timed
        if prof is not None:
    ...
    def launch_bf16_prof(self, spec, a, b, out, prof):
        """Launch an instrumented BF16 kernel, writing wait events into `prof`.

        `prof` is a uint64 CUDA tensor laid out as one slice per cluster; see
        PROF_SLICE_U64 in the instrumented kernel. Not cached, because these
